...
namespace TweetBookAPI.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<AuthenticationResult> RegisterAsync(string email, string password)
        {
            ...
            {
                return new AuthenticationResult { Errors = new[] { "User with this email already exist" } };
            }
        ...
        public async Task<AuthenticationResult> RefreshTokenAsync(string token, string refreshToken)
        {
            ...
            if (storedRefreshToken == null)
                return new AuthenticationResult { Errors = new[] { "The refresh token doesn't exist" } };

            if (DateTime.UtcNow > storedRefreshToken.ExpiryDate)
                return new AuthenticationResult { Errors = new[] { "the refresh token has expired" } };

            if (storedRefreshToken.Invalidated)
                return new AuthenticationResult { Errors = new[] { "The refresh token is invalidated" } };

            if (storedRefreshToken.Used)
                return new AuthenticationResult { Errors = new[] { "The refresh token is already used" } };
