...
class App extends Component {
  ...
  async componentDidMount(){
    await base('Opportunities').select({
    ...
    this.state.records.forEach(record => {
      ...
      if(!recordStatus.filter(status => status.title === record.fields['Status']).length > 0){    
        recordStatus.push({
    ...
    return (
      <div className="main">
        <div className="chart">
         <h1>Sales CRM</h1>
          <PieChart 
            data={this.state.recordStatus}
          />
        </div>
        <div className="legend">
            <ul>
            {this.state.recordStatus.length > 0 ? (this.state.recordStatus.map(status=> 
              <li>
                <p className="statusTitle" style={{backgroundColor: status.color}}>{status.title}</p>
